...
void setup()
{
  ...
  simUGen = new PhyUGen(22050,baseFrameRate); //<>// //<>//
   ...
   simUGen.getMdl().createMatSubset("gnd");
    simUGen.getMdl().addGround3D("gnd0", new Vect3D(0.,0.,0.));
    simUGen.getMdl().addMatToSubset("gnd0","gnd");
    simUGen.getMdl().createMatSubset("str");
    for(int i= 0; i< nbmass; i++)
    {
      simUGen.getMdl().addMass2DPlane("str"+i, m, new Vect3D(dist*(i+1),0.,0.), new Vect3D(0.,0.,0.));
      simUGen.getMdl().addMatToSubset(  "str"+i,"str");
   }
    simUGen.getMdl().addGround3D("gnd1", new Vect3D(dist*(nbmass+1),0.,0.));
    simUGen.getMdl().addMatToSubset("gnd1","gnd");
    
    simUGen.getMdl().createLinkSubset("sprd");
    for(int i= 0; i< nbmass-1; i++)
    {
      simUGen.getMdl().addSpringDamper3D("sprd"+i, l0, k, z, "str"+i, "str"+(i+1));
      simUGen.getMdl().addLinkToSubset("sprd"+i,"sprd");
    }
    simUGen.getMdl().addSpringDamper3D("sprdg0", l0, k, z, "gnd0", "str0");
    simUGen.getMdl().addSpringDamper3D("sprdg1", l0, k, z, "gnd1", "str"+(nbmass-1));
...
listeningPoints[0] = "str4";

    simUGen.setListeningPoint(listeningPoints);
  ...
  midiCtrls.add(MidiController.addMidiController(simUGen.getMdl(),1, 0.01, 0.3, "sprd", "stiffness",0.05));
  midiCtrls.add(MidiController.addMidiController(simUGen.getMdl(),2, 0.0001, 0.1, "sprd", "damping",0.05));
  midiCtrls.add(MidiController.addMidiController(simUGen.getMdl(),3, 0.5, 1.5, "str", "mass",0.05));
